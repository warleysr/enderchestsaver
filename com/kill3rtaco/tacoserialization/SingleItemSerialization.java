...
public class SingleItemSerialization {
	...
	private static JSONObject serializeItems(ItemStack items, boolean useIndex, int index) {
		try {
			...
			JSONObject bookMeta = null, armorMeta = null, skullMeta = null, fwMeta = null, potionMeta = null;
			if(mat == Material.BOOK_AND_QUILL || mat == Material.WRITTEN_BOOK) {
				...
				PotionMeta pm = (PotionMeta) items.getItemMeta();
				if (pm.hasCustomEffects()) {
					potionMeta = new JSONObject();
					String effects = PotionEffectSerialization.serializeEffects(pm.getCustomEffects());
					potionMeta.put("effects", effects);
				}
			}
			if(hasMeta) {
			...
			if (potionMeta != null && potionMeta.length() > 0)
				values.put("potion-meta", potionMeta);
			return values;
...
public static ItemStack getItem(JSONObject item, int index) {
		try {
			...
			if((mat == Material.BOOK_AND_QUILL || mat == Material.WRITTEN_BOOK) && item.has("book-meta")) {
				...
				stuff.setItemMeta(meta);
			} else if (mat == Material.POTION && item.has("potion-meta")) {
				JSONObject potionMeta = item.getJSONObject("potion-meta");
				String serializedEffects = (String) potionMeta.get("effects");
				Collection<PotionEffect> effects = PotionEffectSerialization.getPotionEffects(serializedEffects);
				PotionMeta meta = (PotionMeta) stuff.getItemMeta();
